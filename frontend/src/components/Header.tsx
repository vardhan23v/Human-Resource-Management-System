...
export default function Header() {
  ...
  useEffect(()=>{
    if(!user) return;
    api('/api/notifications').then((r:any)=> setNotifCount(r.data.filter((n:any)=>!n.is_read).length)).catch(()=>{});
      ...
      <div className="container topnav-inner">
        <div style={{ display:'flex', alignItems:'center', gap:24 }}>
          <Link to="/directory" style={{ display:'flex', alignItems:'center', gap:10, textDecoration:'none', color:'inherit' }}>
            <div style={{ width:32, height:32, borderRadius:8, background:'var(--accent)', color:'white', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, fontFamily:'var(--font-display)' }}>D</div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          {['ADMIN','HR'].includes(user.role) && (
            <div style={{ position:'relative' }}>
              ...
              {results.length>0 && (
                <div style={{ position:'absolute', top:40, left:0, right:0, background:'white', borderRadius:12, boxShadow:'var(--shadow-lg)', border:'1px solid var(--neutral-200)', overflow:'hidden', zIndex:30 }}>
                  {results.map((r:any)=>(
                    <div key={r.id} onClick={()=>{ nav(`/profile/${r.id}`); setQ(''); setResults([]); }} style={{ padding:'10px 12px', display:'flex', alignItems:'center', gap:10, cursor:'pointer', borderBottom:'1px solid var(--neutral-100)' }}>
          ...
          <button onClick={()=> nav('/notifications')} style={{ position:'relative', background:'white', border:'1px solid var(--neutral-200)', width:36, height:36, borderRadius:999, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
            <span>🔔</span>{notifCount>0 && <span style={{ position:'absolute', top:-2, right:-2, background:'var(--danger)', color:'white', fontSize:10, fontWeight:800, width:18, height:18, borderRadius:999, display:'flex', alignItems:'center', justifyContent:'center' }}>{notifCount}</span>}
          </button>
          <div style={{ position:'relative' }}>
            ...
            {show && (
              <div style={{ position:'absolute', right:0, top:44, width:200, background:'white', borderRadius:12, boxShadow:'var(--shadow-lg)', border:'1px solid var(--neutral-200)', overflow:'hidden', zIndex:30 }}>
