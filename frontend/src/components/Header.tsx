        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          {['ADMIN','HR'].includes(user.role) && (
            <div style={{ position:'relative' }}>
              <input id="global-search" value={q} onChange={e=>setQ(e.target.value)} placeholder="Search ⌘K" style={{ width:220, padding:'8px 12px 8px 32px', borderRadius:999, border:'1px solid var(--hairline)', background:'var(--card)', fontSize:13, outline:'none' }} />
              <span style={{ position:'absolute', left:10, top:9, color:'var(--neutral-400)', fontSize:12 }}>⌘K</span>
              {results.length>0 && (
                <div className="dropdown" style={{ position:'absolute', top:40, left:0, right:0, background:'var(--card)', borderRadius:12, boxShadow:'var(--shadow-lg)', border:'1px solid var(--hairline)', overflow:'hidden', zIndex:30 }}>
                  {results.map((r:any)=>(
