...
export default function Settings(){
  ...
  return (
    <div className="container" style={{ paddingTop:24, paddingBottom:40 }}>
      <h2 style={{ margin:'0 0 16px' }}>Org Settings (Admin)</h2>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        <div className="card">
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <button className="btn btn-primary" onClick={async()=>{ await api('/api/org-settings',{method:'PATCH', body:JSON.stringify(settings)}); alert('Saved'); }}>Save Settings</button>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button className="btn btn-primary btn-sm" onClick={async()=>{ await api('/api/holidays',{method:'POST', body:JSON.stringify(newHoliday)}); setNewHoliday({date:'',name:''}); load(); }}>Add</button>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginTop:16 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr', gap:8, marginTop:12 }}>
          ...
          <button className="btn btn-primary btn-sm" onClick={async()=>{ await api('/api/leave/types',{method:'POST', body:JSON.stringify(newType)}); load(); }}>Create</button>
        </div>
      </div>
    </div>
  );
